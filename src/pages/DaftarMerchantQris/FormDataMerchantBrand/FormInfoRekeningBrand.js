...
const FormInfoRekeningBrand = () => {
    ...
    const [isLoadingInfoRekeningBrand, setIsLoadingInfoRekeningBrand] = useState(false)
    ...
    async function addFormDaftarSettlementHandler(profileId, settleGroupId, merchantNou, userNou, bankId, noRek, namaPemilikRek, email, merchSettleId, step) {
        try {
            setIsLoadingInfoRekeningBrand(true)
            const auth = "Bearer " + getToken()
            ...
            if (getData.status === 200 && getData.data.response_code === 200 && getData.data.response_new_token === null) {
                if (step === 201) {
                    setIsLoadingInfoRekeningBrand(false)
                    history.push(`/daftar-merchant-qris`)
                } else if (step === 300) {
                    history.push(`/detail-merchant-brand/${profileId}`)
                } else if (step === 2) {
                    history.push(`/formulir-info-usaha-brand/${profileId}`)
                } else {
                    history.push(`/form-dokumen-usaha-brand-badan-usaha/${profileId}`)
                }
            } else if (getData.status === 200 && getData.data.response_code === 200 && getData.data.response_new_token !== null) {
                setUserSession(getData.data.response_new_token)
                if (step === 201) {
    ...
    return (
        <>
            <div className="main-content mt-5" style={{padding: "37px 27px 37px 27px"}}>
                <span className='breadcrumbs-span'><span onClick={() => history.push('/')} style={{ cursor: "pointer" }}>Beranda</span> &nbsp;<img alt="" src={breadcrumbsIcon} /> &nbsp;<span onClick={() => setShowModalSimpanData(true)} style={{ cursor: "pointer" }}>Daftar merchant</span> &nbsp;<img alt="" src={breadcrumbsIcon} /> &nbsp;<span style={{ cursor: "pointer" }}>Merchant Brand</span> &nbsp;<img alt="" src={breadcrumbsIcon} /> &nbsp;<span style={{ cursor: "pointer" }}>Tambah merchant</span></span>
                ...
                <div className='base-content mt-4'>
                    <div style={{ fontFamily: "Exo", fontSize: 16, fontWeight: 600 }} className=''>Informasi Rekening Brand</div>
                    ...
                    <div className='d-flex justify-content-between align-items-center mt-4 pb-4' >
                        <button 
                            ...
                            onClick={() => addFormDaftarSettlementHandler(dataMerchantSettlement?.mprofile_id === null ? id : dataMerchantSettlement?.mprofile_id, settleGroup, merchantNou, userNou, selectedDataBank.length !== 0 ? selectedDataBank[0].value : 0, inputHandle.noRek, inputHandle.namaPemilikRek, dataMerchantSettlement?.mprofdtl_email === null ? "" : dataMerchantSettlement?.mprofdtl_email, dataMerchantSettlement?.mqrismerchsettle_id === null ? 0 : dataMerchantSettlement?.mqrismerchsettle_id, dataMerchantSettlement?.mprofdtl_bustype === 1 ? 3 : 2)}
                        >
                            Sebelumnya
                        </button>
                        <button 
                ...
                <Modal.Body style={{  width: "100%", padding: "12px 24px" }}>
                    ...
                    <div className="d-flex justify-content-center mt-2 mb-3">
                        ...
                        <Button onClick={() => addFormDaftarSettlementHandler(dataMerchantSettlement?.mprofile_id === null ? id : dataMerchantSettlement?.mprofile_id, settleGroup, merchantNou, userNou, selectedDataBank.length !== 0 ? selectedDataBank[0].value : 0, inputHandle.noRek, inputHandle.namaPemilikRek, dataMerchantSettlement?.mprofdtl_email === null ? "" : dataMerchantSettlement?.mprofdtl_email, dataMerchantSettlement?.mqrismerchsettle_id === null ? 0 : dataMerchantSettlement?.mqrismerchsettle_id, 201)} style={{ fontFamily: "Exo", color: "black", background: "var(--palet-gradient-gold, linear-gradient(180deg, #F1D3AC 0%, #E5AE66 100%))", maxHeight: 45, width: "100%", height: "100%", fontWeight: 700, border: "0.6px solid var(--palet-pengembangan-shades-hitam-80, #383838)" }}>
                            {isLoadingInfoRekeningBrand ? (<>Mohon tunggu... <FontAwesomeIcon icon={faSpinner} spin /></>) : `Simpan`}
                        </Button>
                    </div>
                </Modal.Body>
            </Modal>
...
export default FormInfoRekeningBrand