...
import { BaseURL, convertToCurrency, convertToRupiah, errorCatch, getRole, getToken, removeUserSession, setUserSession } from "../../function/helpers";
...
export default () => {
  ...
  const handleQueryPaylinkChange = event => {
    ...
    } else if (!event.target.checked && queryPaylink.includes(event.target.value)) {
  ...
  async function paylinkChartHandler(query) {
    try {
      ...
      const paylinkChartData = await axios.post("/Home/GetFeePaymentLinkChart", {data: dataParams}, { headers: headers });
      console.log(paylinkChartData.data.response_data, 'paylink chart');
      if (paylinkChartData.status === 200 && paylinkChartData.data.response_code === 200 && paylinkChartData.data.response_new_token.length === 0) {
        setPaylinkChartData([{amount: 0, date: ""}, ...paylinkChartData.data.response_data])
        setPendingPaylink(false)
      } else if (paylinkChartData.status === 200 && paylinkChartData.data.response_code === 200 && paylinkChartData.data.response_new_token.length !== 0) {
  ...
  async function filterPaylinkHandler(dateId, periode, query) {
    try {
      ...
      const filterPaylinkChart = await axios.post("/Home/GetFeePaymentLinkChart", {data: dataParams}, { headers: headers });
      // console.log(filterPaylinkChart, 'fee partner handler');
      if (filterPaylinkChart.status === 200 && filterPaylinkChart.data.response_code === 200 && filterPaylinkChart.data.response_new_token.length === 0) {
        setPaylinkChartData([{amount: 0, date: ""}, ...filterPaylinkChart.data.response_data])
        setPendingPaylink(false)
      } else if (filterPaylinkChart.status === 200 && filterPaylinkChart.data.response_code === 200 && filterPaylinkChart.data.response_new_token.length !== 0) {
          ...
          <Row>
            ...
            <Col lg={3}>
              <div className="card-information base-content">
                ...
                <p className="p-amount">{convertToRupiah(settlementTransaction.total_biaya_payment_link)}</p>
              </div>
            </Col>
          ...
          <div className="paylink-section">
            ...
            <div className="base-content mt-3">
              <Row className="mt-4">
                <Col xs={3}>
                  <span>Pilih Periode</span>
                  <Form.Select name='periodePaylinkChart' value={inputHandle.periodePaylinkChart} onChange={(e) => handleChangePaylinkChart(e)}>
                    <option defaultChecked>Pilih Periode</option>
                    {periodik.map((times, idx) => {
                      return (
                        <option key={idx} value={times.value}>{times.time}</option>
                ...
                <Col xs={3}>
                  <span>Pilih Partner</span>
                    <div className="dropdown">
                        <button style={{width: "225px", height: "44px", padding: "12px", borderRadius: "8px", backgroundColor: "#ffffff", border: "1px solid #C4C4C4"}} className="d-flex justify-content-between align-items-center" name="query" onClick={showCheckboxesPaylink} value="none" >
                          <div>Semua</div> <span ><img src={chevron} alt="chevron" style={{fontSize: "5px"}} /></span>
                        </button>
                      {expanded && (
                        <div
                        ...
                        >
                          {listPartner.map((item, idx) => {
                            return (
                              <div key={idx} className="d-flex align-items-center block m-1">  
                                <input type="checkbox" name="query" checked={isCheckedPartner[item.value]} value={item.value} onChange={handleQueryPaylinkChange} />
                                <label className="mx-1 list" htmlFor="query">{item.label}</label>
                              </div> 
                            )
                          })}
              ...
              <Row className='my-3'>
                <Col xs={3}>
                  <Row>
                    <Col xs={6} style={{ width: "unset", padding: "0px 15px" }}>
                      <button
                          onClick={() => filterPaylinkHandler(inputHandle.periodePaylinkChart, dateRangePaylink, queryPaylink)}
                          className={(dateRangePaylink.length !== 0 && inputHandle.periodePaylinkChart.length !== 0 || inputHandle.periodePaylinkChart.length !== 0 && queryPaylink.length !== 0) ? "btn-ez-on" : "btn-ez"}
                          disabled={inputHandle.periodePaylinkChart === 0 && queryPaylink.length === 0}
                      >
                          Terapkan
                      </button>
                    </Col>
                    <Col xs={6} style={{ width: "unset", padding: "0px 15px" }}>
                      <button
                          ...
                          className={(dateRangePaylink.length !== 0 && inputHandle.periodePaylinkChart.length !== 0 || inputHandle.periodePaylinkChart.length !== 0 && queryPaylink.length !== 0) ? "btn-reset" : "btn-ez"}
                          disabled={inputHandle.periodePaylinkChart === 0 && queryPaylink.length === 0}
                      >
                          Atur Ulang
                      </button>
