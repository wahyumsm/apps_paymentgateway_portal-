...
function InvoiceEwallet() {

    const history = useHistory()
    const access_token = getToken()
    const user_role = getRole()
    const [dataInvoiceEwallet, setDataInvoiceEwallet] = useState({})
    ...
    const [showModalKonfirmasiInvoiceEwallet, setShowModalKonfirmasiInvoiceEwallet] = useState(false)
    const [invoiceNumber, setInvoiceNumber] = useState('')
    const [inputHandle, setInputHandle] = useState({})
    const [totalAmount, setTotalAmount] = useState(0)
    const [taxTotalAmount, setTaxTotalAmount] = useState(0)
    const [selectedPartnerInvoiceEwallet, setSelectedPartnerInvoiceEwallet] = useState([])
    ...
    function toPreviewInvoice(objData) {
        ...
        setShowModalKonfirmasiInvoiceEwallet(true)
    }

    function resetButtonHandle() {
        setSelectedMonthYear("")
        setSelectedPartnerInvoiceEwallet([])
        setInvoiceDate("")
    ...
    async function generateInvoiceEwallet(monthYear, partnerId, dateInv, includeZeroAmount, isSave) {
        try {
            const auth = 'Bearer ' + getToken();
            ...
            if (invoiceData.status === 200 && invoiceData.data.response_code === 200 && invoiceData.data.response_new_token === null) {
                ...
                setDataInvoiceEwallet(invoiceData.data.response_data)
            ...
            if (error.response.status === 400 && error.response.data.response_code === 400 && error.response.data.response_message === "Data not found!") {
                ...
                setDataInvoiceEwallet({})
            }
        }
    }

    const SaveAsPDFHandler = () => {
        generateInvoiceEwallet(selectedMonthYear, selectedPartnerInvoiceEwallet.length !== 0 ? selectedPartnerInvoiceEwallet[0].value : "", invoiceDate, isIgnoreZeroAmount, true)
            ...
            .then((dataUrl) => {
                ...
                img.onload = () => {
                    ...
                    for (let page = 0; page < nPages; page++) {
                        ...
                        // data diatas 15
                        pdf.addImage(imgData, imageType, (dataInvoiceEwallet.inv_products.length < 11 ? 60 : 90), (dataInvoiceEwallet.inv_products.length < 9 ? 75 : dataInvoiceEwallet.inv_products.length < 11 ? 70 : 75), (dataInvoiceEwallet.inv_products.length <= 10 ? pdfWidth*0.75 : pdfWidth*0.6), (dataInvoiceEwallet.inv_products.length <= 10 ? pageHeight*0.75 : pageHeight*0.55));
                    }
                    // Output / Save
                    pdf.save(`invoice-disbursement-${dataInvoiceEwallet.partner_detail.partner_name}-${dataInvoiceEwallet.inv_date}.pdf`);
                    setShowModalKonfirmasiInvoiceEwallet(false)
                };
            })
            .catch((error) => {
    ...
    return (
        <div className="main-content mt-5" style={{padding: "37px 27px 37px 27px"}}>
            <span className='breadcrumbs-span'><Link to={"/"}>Beranda</Link>  &nbsp;<img alt="" src={breadcrumbsIcon} />  &nbsp;Invoice E-Wallet</span>
            <div className='head-title'>
                <h2 className="h5 mb-3 mt-4" style={{ fontFamily: "Exo", fontSize: 18, fontWeight: 700 }}>Invoice E-Wallet</h2>
            </div>
            <div className='main-content'>
                <div className='riwayat-dana-masuk-div mt-4'>
                    <div className='base-content mt-3 mb-3'>
                            ...
                            <Col xs={6} className="d-flex justify-content-start align-items-center">
                                ...
                                <div className="dropdown dropTopupPartner">
                                    <ReactSelect
                                        ...
                                        // allowSelectAll={true}
                                        value={selectedPartnerInvoiceEwallet}
                                        onChange={(selected) => setSelectedPartnerInvoiceEwallet([selected])}
                        ...
                        <button
                            onClick={() => generateInvoiceEwallet(selectedMonthYear, selectedPartnerInvoiceEwallet.length !== 0 ? selectedPartnerInvoiceEwallet[0].value : "", invoiceDate, isIgnoreZeroAmount, false)}
                            className={(selectedMonthYear.length === 0 || selectedPartnerInvoiceEwallet.length === 0 || invoiceDate.length === 0) ? 'btn-off' : 'add-button'}
                            style={{ maxWidth: 'fit-content', padding: 7, height: 40, marginRight: 20 }}
                            disabled={(selectedMonthYear.length === 0 || selectedPartnerInvoiceEwallet.length === 0 || invoiceDate.length === 0) ? true : false}
                        >
                            Generate
                        </button>
                        <button
                            onClick={() => resetButtonHandle()}
                            className={(selectedMonthYear.length !== 0 && selectedPartnerInvoiceEwallet.length !== 0 && invoiceDate.length !== 0) ? "btn-reset" : "btn-ez-reset"}
                            style={{ maxWidth: 'fit-content', padding: 7, height: 40, verticalAlign: "middle" }}
                            disabled={(selectedMonthYear.length !== 0 && selectedPartnerInvoiceEwallet.length !== 0 && invoiceDate.length !== 0) ? false : true}
                        >
                            Atur Ulang
                        </button>
                        <div id='tableInvoice'>
                                ...
                                <table className='table table-bordered responsive' style={{ tableLayout: 'fixed' }}>
                                    {/* <tbody> */}
                                        ...
                                        <tr style={{ borderBottom: 'hidden', borderTop: 'solid', borderLeft: 'solid', borderRight: 'solid' }}>
                                            ...
                                            <td style={{ fontWeight: 700 }}>{dataInvoiceEwallet.inv_date ? dataInvoiceEwallet.inv_date : "-"}</td>
                                        </tr>
                                        ...
                                        <tr style={{ borderBottom: 'hidden', borderLeft: 'solid', borderRight: 'solid' }}>
                                            ...
                                            <td style={{ borderRight: 'hidden' }}>:</td>
                                            <td style={{ fontWeight: 700 }}>{dataInvoiceEwallet.partner_detail ? dataInvoiceEwallet.partner_detail.partner_name.toUpperCase() : "-"}</td>
                                        </tr>
                                        <tr style={{ borderBottom: 'hidden', borderLeft: 'solid', borderRight: 'solid', width: '50%' }}>
                                            ...
                                            <td style={{ borderRight: 'hidden', verticalAlign: 'baseline' }}>:</td>
                                            <td style={{ paddingRight: 50, wordBreak: 'break-word', whiteSpace: 'normal', verticalAlign: 'baseline', fontWeight: 700 }}>{dataInvoiceEwallet.partner_detail ? dataInvoiceEwallet.partner_detail.partner_address.toUpperCase() : "-"}</td>
                                        </tr>
                                        <tr style={{ borderBottom: 'solid', borderLeft: 'solid', borderRight: 'solid' }}>
                            ...
                            <div className='div-table' style={{ paddingBottom: 20, marginBottom: 20, display: "flex", justifyContent: "center" }}>
                                <Table bordered>
                                    ...
                                    <tbody className="table-group-divider">
                                        {
                                            dataInvoiceEwallet.inv_products ?
                                            dataInvoiceEwallet.inv_products.map((item, idx) => {
                                                return (
                                        ...
                                        <tr style={{ fontWeight: 700 }}>
                                            ...
                                            <td style={{ borderRight: 'hidden', borderBottom: 'hidden', textAlign: "end" }}>: Rp</td>
                                            <td style={{ textAlign: "end", width: 200, borderRight: 'hidden', borderBottom: 'hidden', borderTop: 'solid' }}>{(dataInvoiceEwallet.inv_dpp !== undefined || dataInvoiceEwallet.inv_ppn !== undefined) ? convertToRupiah((totalAmount + taxTotalAmount), true, 2).slice(3) : "0"}</td>
                                        </tr>
                                    </tbody>
                                </Table>
                            </div>
                            <div style={{ fontSize: 13, fontWeight: 700 }}>
                                <table style={{ width: '100%', backgroundColor: 'rgb(242, 242, 242)', fontStyle: 'italic' }}>
                                    <tr>
                                        <td>Terbilang: {(dataInvoiceEwallet.inv_dpp !== undefined || dataInvoiceEwallet.inv_ppn !== undefined) ? terbilangDisbursement((totalAmount + taxTotalAmount).toFixed(2)).toUpperCase() + " RUPIAH" : "NOL RUPIAH"}</td>
                                    </tr>
                                </table>
                                <div>Remark:</div>
                        ...
                        <div style={{ display: "flex", justifyContent: "end", marginRight: -15, width: "unset", padding: "0px 15px" }}>
                            <button
                                ...
                                className={(Object.keys(dataInvoiceEwallet).length === 0) ? "btn-off mb-3" : 'add-button mb-3'}
                                style={{ maxWidth: 'fit-content' }}
                                disabled={Object.keys(dataInvoiceEwallet).length === 0}
                            >
                                Preview Invoice
                            </button>
                        </div>
            ...
            {/* Modal invoice */}
            <Modal className='modal-invoice' show={showModalKonfirmasiInvoiceEwallet} onHide={() => setShowModalKonfirmasiInvoiceEwallet(false)} style={{ borderRadius: 8 }}>
